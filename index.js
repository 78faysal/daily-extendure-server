...
async function run() {
  try {
    ...
    app.get('/statistics', varifyToken, async(req, res) => {
      ...
      const customersInfo = customers?.map(item => ({buyer: item?.buyer, buyerName: item?.buyerName, buyerImage: item?.buyerImage}));
      ...
      const totalAmount = prices.length > 0 ? prices?.reduce((total, current) => total + current) : 0;
